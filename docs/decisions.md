...
Initial choices were recorded at A0 on 2026-09-20; statuses below are current
as of 2026-09-24 unless explicitly called historical.

| Decision | Reason and tradeoff | State |
|---|---|---|
| Follow assessment outcomes and plan task order | Keeps increments reviewable and defensible | Active; local implementation remains incremental |
| Present the project as an enterprise-grade RAG system | Document Q&A is one capability among ingestion, retrieval, grounded generation and deployment | Implemented locally in part; deployment and evaluation remain |
| ECS Fargate + FastAPI | User-selected container API with explicit runtime/worker control; has idle cost and networking overhead | Selected; not deployed |
| Private S3 + PostgreSQL/pgvector | Store raw documents separately from transactional metadata/vectors | Backend adapters implemented; PostgreSQL is intended for the user's EC2 instance. No project AWS resources provisioned |
| Amazon Bedrock through separately authorized account credentials | Matches the user's separate-account access situation; adds ownership and billing boundaries | Bounded work-account probes passed in A2; ECS credentials are not configured |
| Layered grounding and server-built citations | Refusal and verifiable source mapping are core; citation existence alone does not prove factual support | Implemented locally and tested with fakes; answer quality awaits evaluation |
| SQS worker conditional; reranker/Guardrails optional | Durable delivery and extra model calls add operational cost/latency | No SQS worker or Guardrails; reranker is implemented and off by default |
| Deterministic local fakes; separately approved live checks | Local tests avoid inference charges; fake success cannot establish real model behavior | Local fake tests exist; bounded model probes passed under A2 |
| Start the API through `uv run main.py`; runtime image installs dependencies only | Keep startup direct without building the project package | Implemented and locally container-tested; no deployment |

Pending: account-owner authorization for ongoing inference, PostgreSQL EC2 sizing/networking, durable worker decision, ingress and approved deployment budget. No secrets belong in these records.
...
Cohere Rerank 3.5 (`cohere.rerank-v3-5:0`). Normal requests use Qwen; the caller
can explicitly select GPT-OSS with `thinking_mode=true`. Automatic complexity
routing is not implemented. This model selection does not expose hidden
reasoning. Retrieval starts cosine top-10 and reranks to top-5 when enabled.
Model IDs have defaults in `backend/src/config.py` with environment
overrides. AWS credentials are read from `AWS_ACCESS_KEY_ID`,
`AWS_SECRET_ACCESS_KEY`, and optional `AWS_SESSION_TOKEN`; no real values belong
in the repository. Bounded model probes passed using the authorized work-account
profile, and Bedrock adapters are implemented. Credential delivery to a deployed
ECS task remains unconfigured.
