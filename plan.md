...
- [ ] **A1 — Configure project.** Create Python/FastAPI package, pinned dependencies, `.env.example`, `.gitignore`, `ruff`, `pytest`, Dockerfile and dev commands. **Gate:** fresh install/import, formatting, sample test, container build pass; secret-scanner or grep clean. Explain container entrypoint. **Current: Python setup validated; BLOCKED on Docker WSL integration for container build.** See `docs/progress.md`.
