...
- [x] **A2 — AWS/Bedrock access probe (approved only).** `scripts/check_access.py`: identify caller account/region safely; check S3/storage/DB planned permissions; tiny real generation and embedding probes for each credential path/model only after consent. Reranker separately. **PASS 2026-09-24 with authorized work profile:** account `451058046921`; Titan V2, Qwen3 32B, GPT-OSS 20B and Cohere Rerank 3.5 passed bounded probes. The earlier test-account denial remains documented in `docs/progress.md`.**
