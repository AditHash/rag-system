...
- [x] **A0 — Inventory and source alignment.** Function: none. Inspect repo, PDF, existing files, Git status; create requirement traceability, progress ledger, and explicit baseline list. **Gate:** no invented implementation status; every mandatory PDF requirement represented. Explain required vs chosen features. See `docs/progress.md`; candidate review pending before A1.
- [ ] **A1 — Configure project.** Create Python/FastAPI package, pinned dependencies, `.env.example`, `.gitignore`, `ruff`, `pytest`, Dockerfile and dev commands. **Gate:** fresh install/import, formatting, sample test, container build pass; secret-scanner or grep clean. Explain container entrypoint.
